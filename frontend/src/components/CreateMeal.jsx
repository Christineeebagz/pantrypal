...
import plus from "./Styling/graphics/pluss-symbol1.svg";
  ...
  const handleSubmitMeal = (event) => {
    ...
    for (let i = 0; i < ingredients.length; i++) {
      ...
      const isDuplicate = ingredients.some(
        (ing, index) =>
          index !== i && ing.name.trim() === ingredient.name.trim()
      );
      if (isDuplicate) {
        alert("Duplicate ingredient names are not allowed.");
        return;
      }
      ...
      .then((mealRes) => {
        ...
        if (mealRes.status === 200) {
          const ingredientPromises = ingredients.map((ingredient) => {
            ...
            return axios
              .post("http://localhost:8070/insertIngredient", requestData)
  ...
  return (
    <div>
      ...
      <div className="ADD-NEW-MEAL">
        <div className="div">
          <a href="welcome.html" className="pantrypal-logo">
            PantryPal.
          </a>
