      ...
      axios.get(`http://localhost:8070/getmeals?id_no=${user.id_no}`)
        .then(response => {
          ...
          response.data.forEach(meal => {
            axios.get(`http://localhost:8070/getIngredients?meal_id=${meal.meal_id}`)
              .then(response => {
                console.log("Existing Ingredients for meal", meal.meal_name, ":", response.data);
                // Update existing ingredients state here
                setExistingIngredients(prevIngredients => [...prevIngredients, ...response.data]);
              })
              .catch(error => {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
  const viewMeal = (mealID, mealName) => {
    ...
    if (!editButton.hasEventListener) {
      editButton.addEventListener("click", () => editMealDetails(mealID, mealName));
...
const handleIngredientChange = (event, mealId, rowIndex, field) => {
  ...
  console.log(`Meal ID: ${mealId}, Row Index: ${rowIndex}, Field: ${field}, New Value: ${value}`);

  setValues(prevValues => {
    // Check for duplicate ingredient names if the field being changed is the ingredient name
    if (field === 'name') {
      const isDuplicateIngredientName = existingIngredients.some((ingredient, index) => 
      ...
      .then(response => {
        ...
        if (response.data && response.data.length > 0) {
          ...
          axios.post(`http://localhost:8070/updateIngredient`, {
              ...
              ingredient_name: field === 'name' ? value : ingredientToUpdate.ingredient_name,
              quantity: field === 'quantity' ? value : ingredientToUpdate.quantity,
          ...
          setExistingIngredients(prevIngredients => {
            ...
            newIngredients[rowIndex] = { ...newIngredients[rowIndex], [field]: value };
...
const editMealDetails = (mealId, mealName) => {
  ...
  for (let i = 1; i < rows.length; i++) {
      ...
      iname.addEventListener('change', (event) => handleIngredientChange(event, mealId, i, 'name'));

      let quantity = document.createElement('input');
      quantity.setAttribute('type', 'number');
      quantity.className = 'column3 ingredient-quantity-input';
      quantity.id = `ingredient-quantity-${i}`;
      quantity.addEventListener('change', (event) => handleIngredientChange(event, mealId, i, 'quantity'));
      
      let unit = document.createElement('select');
      unit.className = 'column4 ingredient-unit-select';
      unit.id = `ingredient-unit-${i}`;
      unit.addEventListener('change', (event) => handleIngredientChange(event, mealId, i, 'unit'));
...
const addRow = (mealId) => {
  ...
  check.addEventListener("click", () => {
    console.log("INSIDE ADD INGRED:", iname.value, quantity.value, unit.value);
...
const deleteIngredient = (event) => {
  ...
  setExistingIngredients((prev) => {
    const newIngredients = prev.filter((ingredient) => ingredient.ingId !== target.parentNode.ingId);
